    ...
    if current_user.id == 1:
        ...
        if form.validate_on_submit():
            ...
            try:
                ...
                print(e)
                flash('Sản phẩm thêm không thành công')
    ...
    if current_user.id == 1:
        ...
        if form.validate_on_submit():
            ...
            try:
                ...
                db.session.commit()
                flash(f'{product_name} được cập nhật thành công')
                print('Product Updated')
                return redirect('/shop-items')
            except Exception as e:
                print('Product not Updated', e)
                flash('Sản phẩm cập nhật không thành công!!!')
...
def delete_item(item_id):
    if current_user.id == 1:
        try:
            ...
            db.session.commit()
            flash('Một sản phẩm đã được xoá')
            return redirect('/shop-items')
        except Exception as e:
            print('Item not deleted', e)
            flash('Sản phẩm không được xoá !!!')
        return redirect('/shop-items')
...
def update_order(order_id):
    if current_user.id == 1:
        ...
        if form.validate_on_submit():
            ...
            try:
                db.session.commit()
                flash(f'Đơn hàng {order_id} cập nhật trạng thái thành công')
                return redirect('/view-orders')
            except Exception as e:
                print(e)
                flash(f'Đơn hàng {order_id} cập nhật trạng thái không thành công')
                return redirect('/view-orders')
